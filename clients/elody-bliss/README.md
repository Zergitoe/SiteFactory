...
Site web professionnel genere par **Site Factory** le 2026-06-30.

## Informations

- **Client** : Elody Bliss
- **Secteur** : Coiffure et maquillage de mariée
- **Style** : beaute-mariage
- **Domaine cible** : https://elodybliss.fr/
- **Slug** : `elody-bliss`
- **Mot-cle principal** : maquilleuse coiffeuse mariage Marseille
- **Ville** : Marseille
...
3. **Root Directory** : `clients/elody-bliss`
...
- Settings -> Domains -> ajoute `https://elodybliss.fr/` et `www.https://elodybliss.fr/`
- Configure les DNS chez le registrar (Vercel donne les instructions)

## Deploiement hebergeur classique (Ionos, OVH, O2Switch, etc.)

1. Upload tout le contenu du dossier via FTP a la RACINE du domaine (`www/`, `public_html/` ou `htdocs/`)
   IMPORTANT : a la racine, jamais dans un sous-dossier (les chemins sont absolus)
2. Le fichier `.htaccess` active automatiquement : cache, gzip, HTTPS redirect, clean URLs (`/contact` -> `contact.html`)
3. Verifie que mod_rewrite est active (active par defaut chez Ionos/OVH/O2Switch)
